...
만약 응답이 HTML 파일이라면, 렌더러 프로세스에게 전달하고 zip 파일 또는 다른 타입의 파일이라면 다운로드
매니저에게 전달한다.
...
이 과정에서 SafeBrowsing이 일어난다. 만약 도메인과 응답 데이터가 악의적인 사이트라고 의심되는 경우에는
네트워크 스레드는 경고창을 띄운다. 추가적으로 민감한 cross-site 데이터를 차단하기위한
Cross Origin Read Blocking(CORB) 체크도 이루어진다.
